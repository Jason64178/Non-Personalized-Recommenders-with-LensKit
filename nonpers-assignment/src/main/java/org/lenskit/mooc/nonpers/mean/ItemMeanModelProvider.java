...
public class ItemMeanModelProvider implements Provider<ItemMeanModel> {
    ...
    public ItemMeanModel get() {
        ...
        Long2DoubleOpenHashMap users = new Long2DoubleOpenHashMap();
        try (ObjectStream<Rating> ratings = dao.query(Rating.class).stream()) {
            for (Rating r: ratings) {
                ...
                if (!movie_rating .containsKey(r.getItemId())){
                    ...
                    users.put(r.getItemId(), 1);
                } else {
                    movie_rating.put(r.getItemId(), r.getValue() + movie_rating.get(r.getItemId()));
                    users.put(r.getItemId(), users.get(r.getItemId()) + 1);
                }
            }
        }
        ...
        Iterator it = movie_rating.keySet().iterator();
        while (it.hasNext()) {
            long key = (long) it.next();
            means.put(key, movie_rating.get(key) / users.get(key));
        }
